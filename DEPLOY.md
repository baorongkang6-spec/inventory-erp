...
- **2026-07-15　空库启用日改为 2026-07-01 + 往来单位（M19）**
  - **启用日**：`.env` 设 `ERP_OPENING_DATE=2026-07-01`（代码默认已是该日）。
  - **往来单位**：客户/供应商合并为 `BusinessPartner`（统一编码 + 客户/供应商角色）；菜单「基础资料 → 往来单位」；对冲选一个单位。
  - **推荐做法（本轮约定空库重开）**：停服务 → `backup.bat` 并异地留存旧库 → 将原 `db.sqlite3` 挪走改名 → `migrate` → `seed_init` → `createsuperuser` → 按 **7/1** 重导期初 → 从 7 月起录业务。**不要**在满数据旧库上盲跑 M19 迁移当生产终态（虽迁移支持回写，本轮以空库为准）。
  - 相关迁移：`masterdata.0005–0006`、`finance.0025–0027`、`sales.0008–0010`、`purchasing.0007–0009` 等（含销售/采购退回类型）。
