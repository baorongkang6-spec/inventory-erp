...
- **2026-07-03　核销/冲销统一业务日期口径（迁移 `0021`）**
  - **变更**：应付核销(`PaymentAllocation`)、应收核销(`ReceiptAllocation`)、票据冲销(`NoteSettlement`)原先没有独立业务日期，往来/票据余额表按 `created_at`（操作时钟）归期——**跨月核销会记到"点核销的那个月"而非付款/收款/票据所属月**，且导致依赖当前系统日期的测试不稳定。现各加 `date` 字段并回填：核销取**付款/收款单 doc_date**、票据冲销取**票据 draw_date**；报表改按 `date` 归期。
  - **迁移 `0021` 会自动回填历史**：仅当某笔核销「操作月 ≠ 付款/收款/票据月」（跨月核销）时，相关往来/票据余额表的「本期减少」所属月份会随之更正到正确月份；同月核销的数据不变。
  - **务必先 `backup.bat`** 再 `update.bat`；升级后抽查一两张跨月核销的往来明细账，确认「本期减少」落在付款/收款月。
- **2026-06-17　应收票据「到期兑付 / 贴现」（迁移 `0020`）**：新增票据变现金两种方式（兑付=票面进银行；贴现=实收净额进银行 + 贴现息记财务费用），都自动生成银行存款日记账（业务类型「票据兑现」）。迁移 `0020` 仅建新表 + 给银行日记账业务类型加「票据兑现」选项，**不改既有数据**，可放心更新。
