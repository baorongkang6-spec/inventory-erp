...
## 当前状态（先读这条）
**项目已上线生产、进入"按需迭代"阶段**（M0–M16+ 均完成，远超 SPEC §11 里程碑）。
接续开发前，务必读 **`docs/DEV_NOTES.md` 末尾的「进度快照」**——里面有最新完成项、已知问题、下一步建议、待定问题（`docs/requirements-checklist.md` 文末清单）。
节奏：**服务层守账务一致性 + 视图层管策略（block_reason）+ 模板按 `can_xxx` 显隐 + 写测试 + 重生成手册 docx/pdf + 提交推送 Gitee**。改完必跑 `uv run python manage.py test`（现 190 全绿）。
