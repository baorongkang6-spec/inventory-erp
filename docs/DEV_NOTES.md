...
- ~~**付款一览「应收票据背书」日期取出票日**~~ **已修（2026-08-31）**：背书 `NoteSettlement.date` 取付款登记 `doc_date`（票据页直接背书取操作日）；迁移 `0031` 将历史误存出票日、登记更晚的背书改回登记日。**UX（2026-09-01）**：背书行「单号/票据号」链到该票使用明细并高亮对应背书；页脚说明本行金额=本次背书额。票据使用明细点发票/订单链到详情并高亮该笔核销（`?settlement=`）。
...
- **P3 一键付款 + 预付筛选（2026-08-05）**：订单详情/列表/进度「登记预付/付款」→ `payment_create?order=`；列表勾选「仅有预付未核销」+ 预付未核销列（`annotate_order_prepaid`）。预付挂单切片完成；对称预收可另开。
