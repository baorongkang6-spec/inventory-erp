...
- ~~**付款一览「应收票据背书」日期取出票日**~~ **已修（2026-08-31）**：背书 `NoteSettlement.date` 取付款登记 `doc_date`（票据页直接背书取操作日）；迁移 `0031` 将历史误存出票日、登记更晚的背书改回登记日。**UX（2026-09-01）**：背书行「单号/票据号」链到该票使用明细并高亮对应背书；页脚说明本行金额=本次背书额。票据使用明细点发票/订单链到详情并高亮该笔核销（`?settlement=`）；采购订单详情说明订单日≠背书登记日。
...
- **M18-2 销售订单（2026-07-15）**：`SalesOrder`/`SalesOrderLine`；出库/发票可挂订单；菜单「销售 → 销售订单」；由订单生成出库/发票（数量不超待发/待开；支持先票后货）；无订单出库/发票仍可用。迁移 `sales.0007`、`finance.0023`。下一步 M18-3 采购订单。
- **M18-3 采购订单（2026-07-15）**：`PurchaseOrder`/`PurchaseOrderLine`；入库/发票可挂订单；菜单「采购 → 采购订单」；由订单生成入库/发票（数量不超待收货/待收票；支持先票后货）；无订单入库/发票仍可用。迁移 `purchasing.0006`、`finance.0024`。前缀 `PO-`。下一步 M18-4 未完成业务补单回挂。
- **M18-4 补单回挂 + 进度简表（2026-07-15）**：销售/采购「补单回挂」按客户/供应商扫未完成且无订单的出库/入库与尚有余额的发票 → 新建订单并回挂（仅改关联与进度，不改金额/库存）；订单列表「进度」查看执行中待发/待开（采购待收/待收票）。无迁移。下一步 M18-5 手册与生产迁移核对。
- **销售退回 / 采购退回（2026-07-15）**：出库方式加「销售退回」（库存↑，成本按不含税）；入库方式加「采购退回」（库存↓，移动加权）；关联销售退回镜像对方采购退回。迁移 `sales.0008` / `purchasing.0007`。红字发票勾稽待定。
- **M19 往来单位 SPEC（2026-07-15）**：`SPEC.md` **§21**——客户/供应商合并为统一编码往来单位（`is_customer`/`is_supplier`）、分阶段迁移、对冲改选单户、账务金额不因合并而变。切片 M19-1~5（M19-1 SPEC 已定）。下一步 **M19-2**：建主表 + CRUD + 从旧客户/供应商生成与待合并清单（单据暂挂旧表）。
...
- **应付暂估与未收票明细口径对齐（2026-07-24）**：明细收票过滤改为与总览一致（已登记、非期初）；作废入库上的收票不再冲总览发出；与总览核对时入库起始日须留空。
- **P1 付款可选挂采购订单/预付（2026-08-05）**：`Payment.purchase_order`；银行付款无票可保存；订单已付口径含预付未核销。票据背书暂不挂单。手册补「先付款后收票」步骤。下一刀：P2 订单详情预付明细 / P3 订单一键付款。
- **P2 订单已付/预付展示（2026-08-05）**：`order_payment_summary`；订单详情显示合同含税/已付/预付未核销/未付 + 关联付款表；进度表加已付/预付未核销/未付列。下一刀：P3 订单一键带出付款。
- **P3 一键付款 + 预付筛选（2026-08-05）**：订单详情/列表/进度「登记预付/付款」→ `payment_create?order=`；列表勾选「仅有预付未核销」+ 预付未核销列（`annotate_order_prepaid`）。预付挂单切片完成；对称预收可另开。**补（2026-09-01）**：带 `?order=` 时付款/收款表单的 `doc_date` 默认取订单日，不再用当天（避免 7 月业务 8 月补录时背书日错记为 8 月）。
- **P4 背书挂单 + 销售预收对称（2026-08-05）**：`NoteSettlement.purchase_order`（含无票背书预付）；`Receipt.sales_order` + 销售订单预收汇总/一键收款/列表筛选。迁移 `0029`。
- **未勾发票的收票/收款计入客户应收（2026-08-18）**：应收余额表原先只认「冲销售发票」，本月收票未核销时只体现已冲期初的部分。现将银行收款未分配额 + 非期初收票「票面−已冲应收」计入客户本期收回（多收期末为负）；总览客户往来与明细账同口径。收款一览票据行「已核销」改为按冲应收而非背书已用。
- **收票预收金额量化到分（2026-08-19）**：SQLite `SUM` 浮点导致余额表/明细账出现长尾 0；`round_money` 统一两位小数。
- **运行期运维手册（2026-08-21）**：新增 `docs/运维手册.md`（文档地图 + 生产远程更新/备份/排障）；已导出 Obsidian `30_代码项目/2026-08-21-关联企业库存ERP-运行期运维与远程操作` 与「文档地图与接续入口」。
