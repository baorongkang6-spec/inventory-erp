...
- ~~**付款一览「应收票据背书」日期取出票日**~~ **已修（2026-08-31）**：背书 `NoteSettlement.date` 取付款登记 `doc_date`；迁移 `0031` 曾误用 `created_at` 批量改日。**更正（2026-09-01，迁移 `0032`）**：按 `付款登记_鸿威达_20260901.xlsx` 恢复 C3 共 16 笔背书实际付款日；展示层 `note_settlement_business_date` 只读落库 `date`。
