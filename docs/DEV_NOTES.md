...
- ~~**付款一览「应收票据背书」日期取出票日**~~ **已修（2026-08-31）**：背书 `NoteSettlement.date` 取付款登记 `doc_date`；迁移 `0031` 曾误用 `created_at` 批量改日。**更正（2026-09-01）**：迁移 `0032`/`0033` 按 Excel 恢复 C3/C1 背书实际付款日；新登记 + 从订单跳转默认订单日，展示只读落库 `date`。
