...
class OutboundPostingTests(TestCase):
    ...
    def test_multiline_partial_oversell_rolls_back(self):
        # 多行：一行正常、一行超卖 → 整单回滚
        from apps.inventory.services import InsufficientStockError
        ...
        with self.assertRaises(InsufficientStockError):
            create_and_post_outbound(
                company=self.c1, user=None, doc_date=date(2026, 6, 5),
                lines=[
                    {"product": self.p, "quantity": Decimal("5")},
                    {"product": p2, "quantity": Decimal("99")},
                ],
            )
        self.assertEqual(SalesOutbound.objects.count(), 0)
        self.assertEqual(
        ...
        self.assertEqual(
            StockBalance.objects.get(company=self.c1, product=p2).quantity, Decimal("1.000")
        )
