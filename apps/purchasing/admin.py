...
from .models import (
    PurchaseInbound, PurchaseInboundLine, PurchaseOrder, PurchaseOrderLine,
)
...
class PurchaseInboundLineInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ("product", "quantity", "unit_price", "amount", "stock_move", "order_line")
    can_delete = False
...
class PurchaseInboundAdmin(admin.ModelAdmin):
    list_display = ("doc_no", "company", "doc_date", "supplier", "purchase_type",
                    "purchase_order", "total_quantity", "total_amount", "status")
...
class PurchaseOrderAdmin(admin.ModelAdmin):
    list_display = ("doc_no", "company", "doc_date", "supplier", "total_taxed",
                    "receive_status", "invoice_status", "status")
    list_filter = ("company", "status", "receive_status", "invoice_status")
    search_fields = ("doc_no",)
    date_hierarchy = "doc_date"
